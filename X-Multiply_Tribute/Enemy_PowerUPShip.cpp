...
Enemy_PowerUPShip::Enemy_PowerUPShip(int x, int y, int powerUpid):Enemy(x,y)
{
	...
	switch (powerUpid)
	{
	case 1: pu = App->powerups->AddPowerUp(App->powerups->laser, position.x+2, position.y + 16, COLLIDER_POWER_UP); break;
	...
	case 6: pu = App->powerups->AddPowerUp(App->powerups->shadow, position.x + 2, position.y + 16, COLLIDER_POWER_UP); break;
